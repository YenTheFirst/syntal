require 'midi_java/consts'
require 'midi_java/utils'

class Array
	#because jruby/java is being weird.
	#java bytes are *signed*, but java treats them as unsigned for purposes of midi
	#jruby doesn't automatically convert for us.
	def to_signed_java_byte
		to_signed_byte.to_java(:byte)
	end
	def to_signed_byte
		pack("C*").unpack("c*")
	end
	def to_unsigned_byte
		pack("c*").unpack("C*")
	end
end
module MIDI
class ChannelMsg < javax.sound.midi.ShortMessage
end
class MetaMsg < javax.sound.midi.MetaMessage
end
class SysexMsg < javax.sound.midi.SysexMessage
end
# The abstract superclass of all MIDI events.
class Event < javax.sound.midi.MidiEvent
    ...
    def initialize(status = 0, delta_time = 0)
		...
		super(@message,@time_from_start)
    end
    def getMessage
    	raise "not implemented here"
    	#puts "getmsg"
    	@message if @message
    	@message=Msg.new(data_as_bytes.to_signed_java_byte)
    end
	def getTick
		#puts "getTick = #{@time_from_start}"
		@time_from_start
	end
...
class ChannelEvent < Event
    ...
    def getMessage
    	#puts "getmsg channel"
    	@message if @message
    	@message=ChannelMsg.new(data_as_bytes.to_signed_java_byte)
    end
    protected :initialize
...
class NoteOn < NoteEvent
	...
	super(NOTE_ON, channel, note, velocity, delta_time)
	    ...
	    #overriding, so NoteOff always comes before susequent on
	def <=>(an_event)
		res = @time_from_start <=> an_event.time_from_start
		res==0 ? 1 : res
	end
    def to_s
	return super <<
...
class SystemCommon < Event
    ...
    def getMessage
    	@message if @message
    	@message=SysexMsg.new(data_as_bytes.to_signed_java_byte)
    end
end
...
class MetaEvent < Event
    ...
    def getMessage
    	#puts "getmsg metal"
    	@message if @message
    	@message=MetaMsg.new(data_as_bytes.to_signed_java_byte)
    end
